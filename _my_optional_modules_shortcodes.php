...
/**
 *
 * Category List (for archives templating)
 * [mom_archives] 
 * Outputs a list of all categories, post count, and a link to the latest post in that category
 * Also follows all rules set up in Module->Exclude to hide categories from the appropriate levels
 *
 */
if( !function_exists( 'mom_archives' ) ) {
	...
	function mom_archives($atts,$content = null){
		...
		echo '<div class="momlistcategories">';
		...
		foreach( $category_ids as $cat_id ) {
			...
			echo '<div><a href="' . esc_url( $link ) . '" title="link to ' . esc_attr( $cat->name ) . '">' . esc_attr( $cat->name ) . '</a><span>' . esc_attr( $cat->count ) . '</span><section>' . esc_attr( $cat->description );
			...
			foreach( $latestpost as $latest ) {
				...
				echo '<article><em><a href="' . esc_url( get_permalink( $latest[ "ID" ] ) ) . '" title="' . esc_attr( $latest[ "post_title" ] ) . '" >' . esc_attr( $latest[ "post_title" ] ) . '</a></em></article>';
