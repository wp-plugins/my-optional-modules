 ...
 * external featured 'images' (and not just image files).
 */
...
if( $myoptionalmodules_nelio ) {
	if ( is_admin() ) {
		...
		function myoptionalmodules_url_metabox( $post ) {
			...
			$myoptionalmodules_url = esc_url ( get_post_meta ( $post->ID , _myoptionalmodules_url() , true ) );
			$myoptionalmodules_alt = sanitize_text_field ( esc_html ( get_post_meta ( $post->ID , '_myoptionalmodules_alt' , true ) ) );
			...
			echo "
			<input type='text' placeholder='ALT attribute' style='width:100%;margin-top:10px;$show_if_img'
				id='myoptionalmodules_alt' name='myoptionalmodules_alt'
				value='$myoptionalmodules_alt' />";
			
			if ( $has_img ) {
				echo '<div id="myoptionalmodules_preview_block">';
			} else {
				echo '<div id="myoptionalmodules_preview_block" style="display:none;">';
			}
			
			echo "<div id='myoptionalmodules_image_wrapper' style='width:100%;max-width:300px;height:200px;overflow:hidden;'>";
			
			if ( $myoptionalmodules_url ) new mom_mediaEmbed ( $myoptionalmodules_url );
			
			echo "
			</div>
			<a id='myoptionalmodules_remove_button' href='#' onClick='javascript:myoptionalmodulesRemoveFeaturedImage();' style='$show_if_img'>Remove featured image</a>
			<script>
			...
			<input type='text' placeholder='Image URL' style='width:100%;margin-top:10px;$hide_if_img'
				id='myoptionalmodules_url' name='myoptionalmodules_url'
				value='$myoptionalmodules_url' />
			<div style='text-align:right;margin-top:10px;$hide_if_img'>
