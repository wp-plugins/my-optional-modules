...
if ( !class_exists ( 'myoptionalmodules_miniloop' ) ) {
	class myoptionalmodules_miniloop {
		...
		function __construct() {
			...
			remove_shortcode ( $miniloop );
			add_shortcode ( $miniloop , array ( $this , 'shortcode' ) );
		}
		...
		function shortcode( $atts ) {
			...
			if( 0 < $post_counter ) {
				...
				if ( $show_link == 1 ) {
					...
					$link_text = '<a class="mediaNotPresent" href="' . get_permalink( $id ) . '"></a>';
				}
				...
				if( $paging ) {
					echo '<div class="loopdeloopNavigation">'; 
	...
	$myoptionalmodules_miniloop = new myoptionalmodules_miniloop();
