	...
	echo '<div class="tinythread"><span class="tinysubject">'.$areatitle.'</span><span class="tinyreplies">Board</span><span class="tinydate">Age</span></div>';
	if(count($getposts) > 0){
		foreach($getposts as $posts){
			if($posts->SUBJECT != ''){$subject =  str_replace('\\\\\\\'','\'',(str_replace('\\\\\\','',(str_replace('\\r','<br />',(str_replace('\\n','<br />',rb_format($posts->SUBJECT))))))));}else{$subject = '<em>no subject</em>';}
			...
			echo $subject.'</a> <i id="'.$posts->ID.'" class="fa fa-plus-square loadme hidden" data="'.$THISPAGE.'?t='.$posts->ID.'"></i><i id="'.$posts->ID.'" class="fa fa-minus-square hideme hidden"></i></span><span class="tinyreplies"><a href="?b='.$posts->BOARD.'">'.$posts->BOARD.'</a></span><span class="tinydate">'.timesincethis($posts->DATE).'  <a href="?t='.$posts->ID.'">##</a></span><div id="load'.$posts->ID.'"></div></div>';
		}
	}
	$i = 0;
