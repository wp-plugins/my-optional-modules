<?php if(!defined('MyOptionalModules')){ die('You can not call this file directly.');}
	...
	foreach($getposts as $posts){
			...
			elseif($posts->TYPE == 'video' && $posts->URL != ''){if($profilevideo == 0){$op_embed = '<iframe src="http://www.youtube.com/embed/'.$posts->URL.'?loon=1&amp;playlist='.$posts->URL.'&amp;controls=0&amp;showinfo=0&amp;autohide=1" width="100%" height="315" frameborder="0" allowfullscreen></iframe>';}elseif($profilevideo ==1){$op_embed = '<a class="rb_yt" data="'.$posts->URL.'" href="http://youtube.com/watch?v='.$posts->URL.'"><img class="ytthumb" src="http://img.youtube.com/vi/'.$posts->URL.'/0.jpg"></a><div class="tinycomment" id="'.$posts->URL.'"></div>';}}
			else{$op_embed = '';}
			...
			if(count($gotReplies) > 0 && $THREAD != ''){
				foreach($gotReplies as $replies){
					...
					elseif($replies->TYPE == 'video' && $replies->URL != ''){if($profilevideo == 0){$reply_embed = '<iframe src="http://www.youtube.com/embed/'.$replies->URL.'?loop=1&amp;playlist='.$replies->URL.'&amp;controls=0&amp;showinfo=0&amp;autohide=1" width="100%" heigh="315" frameborder="0" allowfullscreen></iframe>';}elseif($profilevideo == 1){$reply_embed = '<a class="rb_yt" data="'.$replies->URL.'" href="http://youtube.com/watch?v='.$replies->URL.'"><img class="ytthumb" src="http://img.youtube.com/vi/'.$replies->URL.'/0.jpg"></a><div class="tinycomment" id="'.$replies->URL.'"></div>';}}
