<?php if(!defined('MyOptionalModules')){ die('You can not call this file directly.');}
		...
		if($_REQUEST['PARENT'] != ''){
			...
			$checkPARENT = $wpdb->get_results("SELECT * FROM $regularboard_posts WHERE ID = $enteredPARENT AND PUBLIC != 2 AND PUBLIC = 1 LIMIT 1");		
				...
				if($timegateactive !== true){
					...
					if($posting == 1 && $THREAD == '' || $currentCountNomber < $maxreplies && $posting == 1 && $THREAD != '' || $AREA == 'post'){
							...
							if($_REQUEST['COMMENT'] == '' && $URL == '') {
								...
								$wpdb->query("INSERT INTO $regularboard_users (ID, DATE, IP, NAME, EMAIL, THREAD, PARENT, BANNED, MESSAGE, LENGTH, KARMA, PASSWORD, HEAVEN, VIDEO, BOARDS, FOLLOWING) VALUES ('','$current_timestamp','$theIP_us32str','','','0','0','1','filling out hidden form areas (likely bot).','0','0','','0','0','','')");
							}elseif($IS_IT_SPAM == 1){
								$wpdb->query("INSERT INTO $regularboard_users (ID, DATE, IP, NAME, EMAIL, THREAD, PARENT, BANNED, MESSAGE, LENGTH, KARMA, PASSWORD, HEAVEN, VIDEO, BOARDS, FOLLOWING) VALUES ('','$current_timestamp','$theIP_us32str','','','0','0','1','Akismet detected you as SPAM.','0','0','','0','0','','')");
							}else{
								if($IS_IT_SPAM == 1) {
								}else{
									...
									if(count($getDuplicate) == 0 || $_REQUEST['editthisthread'] != '' ){
										if($_REQUEST['EMAIL'] == 'roll'){
											$enteredEMAIL = $randnum1 + $randnum2; 
										...
										if($_REQUEST['REPLYTO'] != ''){
											...
											$checkreplyto = $wpdb->get_results("SELECT ID FROM $regularboard_posts WHERE ID = $checkrepliedto AND BOARD = '".$BOARD."' AND PUBLIC != 2 AND PUBLIC = 1 LIMIT 1");
										...
										if($_REQUEST['editthisthread'] != '' && isset($_POST['FORMSUBMIT']) || 
										   $_REQUEST['admin_ids'] != '' && isset($_POST['FORMSUBMIT']) ){
												...
												if($ISMODERATOR !== true){
													...
													$checkPass = $wpdb->get_results("SELECT * FROM $regularboard_posts WHERE PASSWORD = '".$checkPassword."' AND ID = $checkID AND PUBLIC != 2 AND PUBLIC = 1");
												...
												if($ISMODERATOR === true){
													$checkID = intval($_REQUEST['editthisthread']);
													$checkPass = $wpdb->get_results("SELECT * FROM $regularboard_posts WHERE ID = $checkID AND PUBLIC != 2 AND PUBLIC = 1");
													if(count($checkPass) > 0){
														foreach($checkPass as $Pass){
															$last = $Pass->LAST;
															$wpdb->update( ''.$regularboard_posts.'',
											...
											$wpdb->query(
												$wpdb->prepare(
													...
													( ID, PARENT, IP, DATE, EMAIL, SUBJECT, COMMENT, URL, TYPE, BOARD, MODERATOR, LAST, STICKY, LOCKED, PASSWORD, REPLYTO, UP, USERID, PUBLIC ) 
													VALUES ( %d, %d, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %d, %d, %s, %d, %d, %d, %d )",
													'',
													$enteredPARENT,
													$theIP_us32str,
													$current_timestamp,
											...
											if(count($checkUserExists) == 0){
												$wpdb->query("INSERT INTO $regularboard_users (ID, DATE, IP, NAME, EMAIL, THREAD, PARENT, BANNED, MESSAGE, LENGTH, KARMA, PASSWORD, HEAVEN, VIDEO, BOARDS, FOLLOWING) VALUES ('','$current_timestamp','$theIP_us32str','','','0','0','8','Karma count','','0','','0','0','','')");
											}
										}
										...
										if(count($getDuplicate) > 0){
											...
											if(count($automute) > 0){
												...
												$wpdb->query("INSERT INTO $regularboard_users (ID, DATE, IP, NAME, EMAIL, THREAD, PARENT, BANNED, MESSAGE, LENGTH, KARMA, PASSWORD, HEAVEN, VIDEO, BOARDS, FOLLOWING) VALUES ('','$current_timestamp','$theIP_us32str','','','0','0','5','unoriginal','10 minutes','','','0','0','','')");
											}
							...
							if($edited == 1){
								$LAST = $wpdb->get_results("SELECT * FROM $regularboard_posts WHERE (ID = $checkID OR PARENT = $checkID) AND PUBLIC != 2 AND PUBLIC = 1 ORDER BY ID DESC LIMIT 1");
							}else{
								$LAST = $wpdb->get_results("SELECT * FROM $regularboard_posts WHERE IP = '".$theIP_us32str."' AND PUBLIC != 2 AND PUBLIC = 1 ORDER BY ID DESC LIMIT 1");
							}
