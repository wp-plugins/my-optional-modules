...
== Changelog ==
= .5.3.9 =
* (code) selected() replaces check/echo functions on module settings pages.
* (code) any javascript that wasn't being configured via options has been moved to its own separate .js and enqueued individually and conditionally.
* // these files include: wowheadtooltips.js (wowhead tooltips), stucktotop.js, stucktobottom.js (navbar), regularboard.js (regular board), lazyload.js (lazy load)
* (code) unused function removed
* (code) improperly closed div on theme takeover settings page fixed
