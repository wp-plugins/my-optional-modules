...
== Changelog ==
= .5.3.9.5 =
* (bugfix) Jump Around deactivation button fixed.
* (bugfix) Hanging Javascript should be fixed (in the footer) If no modules that use emmbedded scripts are active, then it shouldn't be there.
* (code)   Some of the SQL items, while stripped, HTMLentitied and sanitized, have also been properly escaped (as well).  (trust no one, Mr. Mulder.)
* (code)   Some functions have been renamed.  This should have no effect on you whatsoever, unless you were plugging into MOM's core (which you probably shouldn't be doing, anyway)
